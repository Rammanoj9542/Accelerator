...
class MongoDBHandler:
    ...
    def getRetrivalConfig(self, clientApiKey, retrivalId):
        try:
            org = self.get_org_id(clientApiKey)
            if org:
                db = self.client[org]
            if db is not None:
                RetrivalConfig = db["RetrivalConfig"]
                RetrivalData = RetrivalConfig.find_one(
                    {"clientApiKey": clientApiKey, "retrivalId": retrivalId}, 
                    {"clientApiKey": 0, "_id": 0, "retrivalId": 0}
                )
                print(RetrivalData)
                return RetrivalData
            print("Error fetching Retrival Config details or missing data")
            return None
        except Exception as e:
            print(f"Error Fetching 'RetrivalData' : {e}")
